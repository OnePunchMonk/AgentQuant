...
| **Measured historical experiment** | Real OHLCV history (yfinance), an actual `run_agent`/epoch execution, with in-sample search Sharpe reported separately from held-out Sharpe. Still a single historical window, not a claim about future/live performance. Without an LLM key, all 6 epochs run the offline `FallbackPlanner` and are expected to (and, in the checked-in `results/harness_evolution_6epochs_results.json`, do) produce identical Sharpe — that's confirmation of correct fail-closed behavior, not an epoch-over-epoch improvement claim; a real evolution comparison needs `ANTHROPIC_API_KEY` (and ideally `TAVILY_API_KEY`) exported per issue #19. | 6-epoch harness evolution runs, each producing a `HarnessConfig` hash + run manifest | `python3 scripts/harness_evolution_6_epochs.py --strategy momentum --output results/harness_evolution_6epochs_results.json` |
